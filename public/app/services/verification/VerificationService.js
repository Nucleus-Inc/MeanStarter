  ...
  angular.module('dashboard').service('Verification', ['$http','Config', ($http,Config) => {
    ...
    return {
      userVerifyEmail : (email) => {
        return $http.get(url_base+'/verifications/user?email=' + email).then((result) => { return result }).catch((err) => { return err })
      },
      userVerifyPhoneNumber : (phoneNumber) => {
        return $http.get(url_base+'/verifications/user?phoneNumber=' + phoneNumber).then((result) => { return result }).catch((err) => { return err })
      },
