          ...
          if (result && result.status) {
            if (isActive) {
              Account.inactivate(id).then(function (res) {
                ...
                Notify.run('Erro ao tentar desativar usuário', 'alert-danger', null, null, null, function (res) {
                  if (res) Socket.emit('admin inactivate', id);
                });
              });
            } else {
              Account.active(id).then(function (res) {
                ...
                Notify.run('Erro ao tentar ativar usuário', 'alert-danger', null, null, null, function (res) {
                  if (res) Socket.emit('admin active', id);
                });
              });
            }
      ...
      link: function link(scope, iElement, iAttrs, ngModelCtrl) {
        ...
        scope.$watch('ngModel', function (value) {
          if (value) {
            ...
            var res = str.match(/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/);
            if (res) {
              //email is match
