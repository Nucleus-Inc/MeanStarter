...
describe('User SignIn', () => {
  it('should successfully login on /users/auth/local/login POST', (done) => {
    chai.request(server)
      .post('/users/auth/local/login')
      .send({
        'email': user.email,
  ...
  it('should fail to login with invalid credentials on /users/auth/local/login POST', (done) => {
    chai.request(server)
      .post('/users/auth/local/login')
      .send({
